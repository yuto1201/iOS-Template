...
- 新しいアプリでは目的・方向性と4つのIdentity値を確定してIdentity bootstrapを完了した後、最初のユーザー向けUI `shape`より前に[App Icon skill](.agents/skills/app-icon/SKILL.md)でシンプルな画像生成候補を2案作り、ユーザーが明示選択した1案だけをAppIconへ組み込む。選択待ちは`blocked:user`とし、独立した非UI作業は止めない。アプリアイコン選択はUI Direction Gateの承認として扱わない。
- 新しいアプリではIdentity bootstrap完了後かつ主要Feature Issueの計画・Claim前に[System Experiences Planning skill](.agents/skills/ios-system-experiences/SKILL.md)を使い、Widget、Live Activities、Dynamic Island、Controls、Siri／App Intentsの5面をすべて`adopt-now`、`defer`、`not-applicable`、`blocked:user`のいずれかへ分類する。採用は一律必須にせず、最終判断はユーザーへ残す。App Icon Issueとは並行できるが、採用するsystem UIの実装Issueは完了済みplanning Issueへ依存し、独立した非UI作業は止めない。
