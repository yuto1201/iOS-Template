...
Promote temporary copy only after reviewing it against actual app behavior. Keep existing-schema values in their files. Preparation-only values have a [versioned partial source format](metadata/preparation-format.md); migrate `metadata/reviewed-draft.md` explicitly through that procedure while preserving the original ledger and unanswered choices. Human-readable state labels are not confirmation evidence or a sealed release package. Never store contact values or credentials.
