...
This contract separates offline drafts, selective metadata saves, release readiness and authenticated submission. A prepared package does not authorize submission. The selective-save entrypoint is `.agents/skills/save-appstore-metadata/scripts/save-appstore-metadata.sh`; it is usable only with an authorized app Issue, configured Team/Bundle, fresh operation-specific preflight and confirmed sources.

## Build archive and upload

`tools/export-appstore-build.sh --project-root <absolute-root> --issue <number> --head-sha <40-hex> --version <x.y[.z]> --build-number <number>` archives the clean fixed Head, exports an IPA, and calls the guarded `appstore.upload_build` runner. The app project and scheme come only from `Config/app-identity.json`. The tool requires a sealed operation contract, matching configured Team and Bundle ID, a fresh `app-store-upload_build.json` preflight, and matching Release Bundle ID, marketing version, and build number before archive. Artifacts and an append-only digest journal are kept under `.artifacts/appstore-builds/<issue>/<attempt>/`.
...
Paths within the table's `metadata/`, `privacy/`, `review/` and `legal/` refer to `App Store/`. Current exact-key YAML schemas do not support every field above: do not add SKU, secondary category, app Apple ID, confirmation states or questionnaire answers as arbitrary YAML keys. Use the versioned `App Store/metadata/preparation.json` only for its defined additional source values, with proofs under `.artifacts/appstore-preparation/`. Preserve the human-readable `reviewed-draft.md`; explicitly review and transcribe values without copying its status labels into evidence. The current validator also fixes `primaryLocale` to `en-US`; a confirmed different primary language is a reported implementation gap, not permission to silently substitute English or loosen validation.
...
Inventory chapters are not save forms. Follow the format's explicit ASC resource mappings: internal module/slug, local legal documents, SDK inventories and restore behavior do not become Apple metadata fields. Source confirmation, registration/account observation, public-page verification and an authorized metadata readback are distinct evidence types. A requested unsupported readback stays unresolved; never synthesize an aggregate Apple form from the fields in a chapter. Supported normalized forms bind the exact remote resource reference into approval, baseline and readback. Private preserved fields additionally require authorized transient observations; matching Keychain labels alone do not establish equal values.
...
These requirements are exercised through the real public entrypoint in `tools/tests/test-appstore-preparation.sh` and the explicit transcription/preservation cases in `tools/tests/test-appstore-preparation-migration.sh`. A fully confirmed non-secret inventory must succeed; each invalid variant must report its cause and affected fields, avoid remote writes, preserve user sources and keep independent preparation available. Matching-existing-app resume, duplicate/conflicting identities, source-digest invalidation, contact/credential redaction and immutable old package/result compatibility remain required. Existing `test-appstore-package.sh` and `test-appstore-skills.sh` independently cover the unchanged complete-package/result/legal gates. Test existence or a focused local pass does not replace #110's immutable targeted repository-test plan/evidence, workflow-only verification, current-Head strict opposite-model review or pre-merge gate. Xcode, Build, Unit Test, Simulator, screenshot and visual evidence are not applicable to this workflow-only implementation.
...
The `submit-appstore-release` skill routes `save` to the dedicated [`save-appstore-metadata` entrypoint](../../.agents/skills/save-appstore-metadata/SKILL.md), and keeps `draft`, `ready` and `submit` on their existing paths. The save writer accepts only `name`, `subtitle`, `description`, `keywords`, `promotionalText` and `whatsNew`; it reads values from confirmed fixed sources and uses only the [pinned `asc` guarded runner](../../specs/architecture.md#72-app-store-connect-api-adapter). `promotionalText` has immediate public-page effect and requires a scoped user approval reference; the other five fields require an editable selected version. Do not feed a partial save to `record-section.sh` or bypass the save gate with manual authenticated browser input. #52 itself performed no Apple operation.

The writer's versioned `appstore-metadata-save-request` contains the owning Issue, executor, exact identity and source revision, current official-requirements check, selected source path/anchor/digest rows, and a complete baseline digest plus `asc://` reference for each selected locale/form. It contains no values. A form digest is SHA-256 of compact UTF-8 JSON with recursively sorted object keys and exact `section`, `locale`, `remoteReference`, `values` keys. The modeled `values` preserve absent versus null versus empty string: an absent required attribute blocks the form before hashing, while null and empty remain distinct. The exact request bytes are bound into each journal event for resume.

### Selective-save preconditions and transaction

Each save attempt must meet all of the following; no whole-package prerequisite is silently weakened in the existing release entrypoint:
...
Use the same fixtures for English and Japanese where applicable. The dedicated entrypoint's fake-runner tests implement these checks without claiming an authenticated App Store Connect save. Use synthetic app data only, including the example request “enter confirmed text; leave screenshots for later.”
...
- Operations that the public App Store Connect API can perform use only the [pinned `asc` guarded runner](../../specs/architecture.md#72-app-store-connect-api-adapter) with a Team key that has the App Manager role. App Privacy and review information remain authenticated-browser sections. The latter needs contact and demo secrets, which `asc` 5.4.0 only accepts in arguments.
...
### Legal-page handoff and verified return

After the user has approved the exact routes and every localized source is `Status: Confirmed`, use `tools/prepare-appstore-legal-handoff.sh render` and the [handoff template](../../.agents/skills/prepare-appstore-assets/templates/legal-page-handoff.md). The request covers exactly English/Japanese support, privacy, and terms sources, their SHA-256 digests and approval references, app facts, the public host/routes, and the required return. It produces copy-ready Markdown for exactly `yuto1201/Web-AppLibrary`; it does not create, edit, approve, or publish anything by itself.

Before Issue creation, search open and closed Web-AppLibrary Issues. The source Issue must declare `github.create_issue`, executor, account, and exact target. Run the external-operation preflight immediately before creation, create at most once, and read back number/URL/title/body/state. On a missing or ambiguous response, reconcile by search/readback instead of retrying. `record-issue` rejects a different repository or changed body. The user—not an agent—controls forwarding the prompt to the Web implementation agent and approving the legal text/publication.

The returned deployment reference and public URLs become release inputs only after live `verify-publication` succeeds. It binds the request, prompt, Web Issue, user action references, source digests, exact HTTPS host/routes, login-free HTTP 200 responses, matching source text/locale, and support/privacy/terms interlinks. A response fixture may verify the tool contract but yields `fixture-validated` with `appStoreEligible: false`; it is never release or App Store evidence. A changed source, route, approval, deployment, or page invalidates the result and requires a fresh handoff or verification as appropriate.
...
The `app-information`, `localization`, `screenshots`, `build`, and `submission` sections enter, save, and read back through `tools/lib/appstore-release-sections.rb` and the pinned `asc` guarded runner. `privacy` and `review-information` retain the selected executor's authenticated-browser procedure. `review-information` stays there because `asc` 5.4.0 `review details-update` takes the demo password and private contact email/phone as command arguments. The API tool reuses #132's full-form localization baseline/save/readback path and #133's VALID processing readback journal; screenshots proceed only with an empty or identical remote set. Each section uses its declared operation's production preflight: `appstore.update_metadata` for app information, localization and screenshots; `appstore.submit_review` for build and submission; `appstore.inspect_app` for both browser sections. The order, sealed-value rule and stop conditions above remain in force.

## Result and resume

`App Store/submission/${VERSION}-result.json` is an ordered, sanitized schema-2 journal. Each entry contains the section ID, `verified` status, an `asc://` remote reference, SHA-256 readback digest, `app-store-connect-api` or `app-store-connect-browser` source, and verification time. It contains no field values or credentials. A schema-1 result is legacy and cannot be silently migrated or resumed; create a new result for a new candidate.

On resume, the selected executor must authenticate again, rerun the next operation's preflight, and recompute all immutable inputs. The API tool re-reads every recorded API section through the runner; the executor supplies fresh sanitized browser readback digests/references for every recorded browser section. Both must match the sealed source and recorded result before `scripts/record-section.sh --resume-readback yes` is valid. Local status alone is not evidence of remote completion.
