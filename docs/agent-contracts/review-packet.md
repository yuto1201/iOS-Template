...
Phase 6 evidence applicabilityを持つpacketでは、reviewerはPhase 5 source proof、source／target contract、release／revision／scope、phase record、source..target Git diff、candidate artifact／configuration／SDK／signing context、全changed pathのimpact／dependency、decision、評価時刻を同じrecordから確認します。`reuse`はexact同一条件だけを支持し、`targeted-reverify`／`expanded-verification`は評価後のtarget passed evidenceがなければunsupportedとします。review approvalはcurrent verifyとapplicability評価の遅い方より後でなければなりません。`evidence-applicability.json`はfinding／assessmentのcanonical aliasとして使えますが、Phase 5での実行をPhase 6再実行と記述しません。
