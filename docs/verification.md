...
planはIssue／Base／Head／contract digest、要求・解決scopeと理由、manifest digest、changed pathsとdiff digest、sorted exact test paths、全AC順のmappingを持ちます。mappingは各ACをexactly once含み、参照のunionがplanのtest集合と一致しなければ生成しません。schema v3 `repository-tests.json`はplan path/digest、resolved scope、current-Head producer、実行結果を持ち、review packet、result publication、PR本文、pre-merge gateは同じplan bytesをimmutable Git入力から再計算します。開発中の対象testは1件300秒、通常完了の`targeted` suiteはaggregate 900秒を上限とし、canonical runnerは安定した最終候補Headで一度だけ使います。runnerは開始前にscope、ordered tests、test count、child／aggregate上限を表示します。独立した長時間entrypointは専用domainへ分離し、変更されていないbootstrap／workflow全体をtest自身の分類だけで巻き込まない一方、変更されたtest自身と対応producerは必ず同じdomainで選択します。`targeted`でmerge、pre-merge、PR本文、workflow E2E、workflow stateのentrypointが選ばれた場合は、各testの`scoped`シナリオだけを記録し、exact Headの成功経路と対象安全境界を確認します。引数なしの直接実行と`head-all`／`base-and-head`は従来どおり全adversarialシナリオを維持します。

[workflow e2e](../tools/tests/test-workflow-e2e.sh)は、隔離したGit remote／fake GitHubでdocumentation-onlyのClaim／Resume、検証公開、review、premerge、merge、cleanup、doneと、無関係なBranch／worktreeの保護を検査します。application経路は別の隔離primaryで実際のClaim toolが生成したcontractのexact bytesを、共有runner fixtureの合成repositoryへ変更せず渡し、`tools/verify-ios-issue.sh`のdraft発行を確認します。同じhelperへapplication Verificationを持たないdocumentation-onlyのClaim contractを渡した場合は、Build前の拒否とfake `xcodebuild`呼出し0件を確認します。このapplication経路はfake Xcode／simctlによるCI実行可能なcontract検査であり、実SimulatorのBuild／Test／画面／visual evidenceではありません。実Simulator検証をCI必須にはせず、canonical native検証は対象application Issueで別途実行します。
