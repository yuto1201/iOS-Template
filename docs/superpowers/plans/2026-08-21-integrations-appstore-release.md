...
**Interfaces:**
- `secret-store.sh put --app caflog --service elevenlabs --environment production --key api-key` reads the value from stdin
- `secret-store.sh check` reports only present or absent
- `run-with-secret.sh --service-name ios-template/caflog/elevenlabs/production/api-key --env ELEVENLABS_API_KEY -- command arguments...`
...
Read the secret into a shell variable, export it under the validated environment variable name only for `exec env`, and unset the shell variable on error. Reject tracing modes and commands containing shell evaluation strings.

- [ ] **Step 6: Expand ignore rules and run tests**

Ignore `Config/Local.xcconfig`, `.env` variants except `.env.example`, private keys, profiles, and generated secret staging directories.
...
```bash
git add .gitignore tools/secret-store.sh tools/run-with-secret.sh tools/tests
...
- `provider-preflight.sh supabase --project-ref ivsauhjnoiurpsriskqe --environment production`
- `provider-preflight.sh cloudflare --account-id 7811bdd81f838a59e4fa269651ae67ec --target example.com`
- `provider-preflight.sh elevenlabs --operation sound-effect`
- `provider-preflight.sh app-store --bundle-id com.yuto.TemplateApp --version 1.0`
...
GitHub requires `yuto1201`. Supabase requires the configured personal Organization and exact Project Ref. Cloudflare requires the configured personal Account ID. App Store requires the app's configured personal Team and Bundle ID. A mismatch exits before any mutation.
...
Test required purpose, duration range, loop boolean, output path, supported audio format, file hash, and rejection of API keys. Test that music `paid_plan_required` becomes one `blocked:ops` result without retry.
...
Claude prepares the prompt and local integration code but delegates generation to Codex. Codex runs identity preflight, invokes the installed ElevenLabs procedure, validates the file, stores it in the feature's Resource directory, and records the sanitized manifest.
...
Test complete fixture, missing Japanese subtitle, description over the current fetched limit, privacy declaration inconsistent with entitlement/SDK scan, missing support URL, missing screenshot family, and secret-like review credentials.
...
The validator fetches current official App Store field limits and screenshot requirements at release preparation time through Codex, caches the sanitized requirement version in `App Store/submission/requirements.json`, and refuses to validate against a cache older than 30 days.
...
**Interfaces:**
- Consumes: audited screenshot requirements, deterministic UI launch states, and the batch Simulator matrix
- Produces final PNG files grouped by locale and required display family

- [ ] **Step 1: Write image-set tests**

Use fixture PNGs to test dimensions, alpha-channel rejection where prohibited, duplicate image hashes, missing locale, ordering, safe-area clipping, and manifest completeness.

- [ ] **Step 2: Implement deterministic capture**

Launch one documented UI state per screenshot with fixed local data, locale, time, and appearance. Capture raw PNGs without secrets, notification banners, debugging overlays, or personal content.
