...
/usr/bin/ruby -rjson -ryaml -rdigest -rtime -ropen3 -rtmpdir -e '
  ...
  keys=%w[schemaVersion purpose prompt model durationSeconds loop loopDiscontinuityThreshold format assetPath licenseNote sha256 generatedAt]
  ...
  refuse("durationSeconds is invalid") unless duration.is_a?(Numeric) && duration.finite? && duration.between?(0.5,600.0)
  looped=value["loop"]
  ...
  refuse("sha256 is invalid") unless expected_digest.is_a?(String) && expected_digest.match?(/\Asha256:[0-9a-f]{64}\z/)
  refuse("audio hash differs") unless expected_digest == "sha256:#{Digest::SHA256.file(asset).hexdigest}"
  begin
    Time.iso8601(value["generatedAt"])
  rescue ArgumentError, TypeError
    refuse("generatedAt is invalid")
  end
  ...
  if looped
    Dir.mktmpdir("ios-template-loop-analysis") do |directory|
      ...
      output,convert_status=Open3.capture2e("/usr/bin/afconvert","-f","WAVE","-d","LEI16@44100",asset,pcm)
  ...
  result={"durationSeconds"=>measured.round(6),"format"=>format,"loopDiscontinuity"=>discontinuity&.round(6),"sha256"=>expected_digest,"status"=>"valid"}
  puts JSON.generate(result.keys.sort.to_h{|key| [key,result[key]]})
' "$repository_root" "$manifest_file" 2>/dev/null || fail 'manifest, media, duration, hash, or loop contract did not validate'
