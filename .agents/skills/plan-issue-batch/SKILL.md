...
3. Split by independently verifiable outcome. Give every Issue Goal, In/Out of scope, ordered `AC-1..n`, exact spec anchors, dependencies, UI verification, Delivery stage with a positive Time budget and reason, Delivery profile with reason, Verification scope with reason when applicable, External operations, User approvals, and an expected write-set. Use exactly one of `type:feature`, `type:regression`, `type:docs`, or `type:release`.
