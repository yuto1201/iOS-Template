...
If a repository suite fails or times out, do not rerun the same Issue／Head／scope directly. Run one selected affected test as the diagnostic and permit at most one explicit retry; a second failure stops the Issue. Never report deferred English, iPad, full-suite, or unexecuted checks as passed.
