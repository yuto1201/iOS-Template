...
For a sealed `Repository-test scope: base-and-head; ` AC declaration, require both revision suites and the packet's canonical `repositoryTestsFile` path/digest before dispatch. Both reviewer routes must distinguish Base baseline/regression evidence from new Head implementation evidence and cite each supported AC's own zero-based `repository-tests.json#acceptanceEvidence/INDEX`. Missing or changed record bytes, another AC's mapping, or Head-only evidence cannot satisfy this route. Keep old Head-only contracts and sealed bytes unchanged; see [the review contract](../../../docs/agent-contracts/review-packet.md#baseとheadを要求するcontract).
