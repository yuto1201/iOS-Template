...
Cursor may aggregate a short progress prefix into the provider envelope's `result` string. Trust only the canonical normalizer: it accepts a unique terminal JSON object only after a bounded valid-UTF-8 prefix with no brace or NUL, discards that prefix, and still applies the complete Result validator and receipt binding. Never copy a JSON suffix by hand, remove a finding, accept multiple/partial objects, or treat a rejected diagnostic as approval.
