...
The producer and review orchestrator own schema-v2 closure and `reviewPacketDigest` validation. `codex` primary maps only to `claude`; `claude` primary maps only to the fixed Codex transport. Both reviewers are noninteractive, read-only, and time-bounded. Do not call either model CLI or transport directly.

3. A valid `approved` result transitions the Issue to `approved-for-merge`. A valid `changes-requested` result transitions it to `changes-requested`; apply only in-scope fixes as the primary agent, repeat affected verification, produce a new Head-bound packet, and request a new opposite-model review. A timeout is `blocked:review`; never self-approve or fall back to another reviewer.
4. Before merge, use the pre-merge gate. It independently requires the same current Head and exact packet-bound evidence in canonical `verify.json` and `review.json`.
