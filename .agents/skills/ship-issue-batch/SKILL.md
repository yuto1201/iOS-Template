...
For a new app's major Feature graph, schedule the System Experiences Planning Issue produced by [`ios-system-experiences`](../ios-system-experiences/SKILL.md) immediately after Identity bootstrap. App Icon and independent non-UI work may run alongside it. Only `adopt-now` surfaces create implementation predecessors; dependent nodes wait for those predecessors to be `done`. A `blocked:user` decision applies 部分blocking to that surface and its descendants, while settled surfaces and unrelated graph components continue.

## Release-phase scheduling

For every ready node, read its sealed `Release-phase binding:` and group only matching release identifier/revision records. Schedule `implementation` after all required previous Phase exits and explicit Issue dependencies are complete. Phase 3 Japanese-iPhone nodes precede Phase 4 English/iPad adaptation; Phase 5 full-quality nodes precede Phase 6 publication nodes. A Phase 5 finding may open a focused earlier-Phase regression lane, but only invalidated descendants wait; unaffected lanes continue. `research` and `draft` can run as read-only work before an exit, and `independent` can run only for the exact sealed non-dependent reason.

Do not use batch order, Issue number, Delivery stage, or merge time as a substitute for the phase record. A sealed Issue without a binding is `legacy-unbound`; preserve its existing dependencies and gates without adding, guessing, or resealing a record. When a major change appends a new release revision, stop descendants bound to the old revision, re-plan from the earliest reopened Phase, and keep completed historical contracts immutable.
...
- Run only one Simulator lifecycle per inherited session at a time. Let `ios-verify` acquire the repository-wide lock and the Mac-wide resource manager enforce four total iPhone/iPad allocations across repositories; other lanes may perform non-Simulator local work meanwhile. A four-case matrix allocates, verifies, saves evidence, releases, and confirms deletion before its next case.
