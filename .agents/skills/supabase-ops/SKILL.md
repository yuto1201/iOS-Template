---
name: supabase-ops
description: Activate and operate Supabase only when a confirmed app specification explicitly requires authentication, synchronization, remote database, or storage. Use for migration-first local files, RLS validation, local testing, or account-bound remote inspection and apply operations from Codex or Claude.
---
...
- Codex and Claude may run local and remote operations. Remote `link`, `pull`, `push`, advisors, and SQL require `external-ops`, exact Organization ID／Project Ref, and the declared Issue executor.

## Remote operation

Before an authenticated operation, have the selected executor run `tools/provider-preflight.sh --executor "$EXECUTOR"` for the exact Issue and environment. Report the configured Organization ID, Project Ref, health, pending migration names, and dry-run result without credentials. Use only the operation declared in the Issue contract.
