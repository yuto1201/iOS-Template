...
Before any Feature Issue starts, confirm that the app-specific `specs/product.md` and `specs/acceptance.md` are both `Status: 確定` and consistent with the Issue acceptance criteria. If either document is missing, not 確定, or inconsistent, have the Issue-selected Codex or Claude executor transition the Issue to `blocked:user`. Do not create its Branch/worktree and do not implement it. After Identity bootstrap, run [`ios-system-experiences`](../ios-system-experiences/SKILL.md) and complete its System Experiences Planning Issue before planning or claiming a major Feature Issue. Evaluate all five surfaces but create implementation dependencies only for `adopt-now`; one unresolved surface causes only partial blocking. In parallel, use [`app-icon`](../app-icon/SKILL.md) and complete its dependent Issue before the first user-facing UI `shape`; do not block independent non-UI work on either selection.

The system-experiences decision and App Icon selection are separate. App Icon may proceed as soon as Identity bootstrap is complete. A system surface classified `adopt-now` must have its architecture/specification dependency merged before a dependent Feature Claim, and any user-facing system UI still follows `ui-direction`; the planning matrix is not UI approval.

## Release-phase handoff

Bootstrap establishes app identity; it does not silently approve a release scope or a later Phase. After the bootstrap Issue is merged and before planning the major Feature graph, use `plan-issue-batch` to create or append the immutable release record under `Config/releases/<release-id>/phase-records/`. New dependent Issues carry exactly one `Release-phase binding:` in an existing Acceptance criterion. The committed Base record, not a working-copy draft, names the release identifier, revision, Phase, sorted scope, route, and work kind.

System Experiences decisions belong to Phase 1 and adopted architecture is refined in Phase 2. The first core implementation normally enters Phase 3 only after those exits and any UI-direction prerequisite are complete. Research/draft work may remain read-only, and a reasoned independent lane may continue, but neither bypasses the previous-Phase implementation gate. A pre-existing sealed Issue with no binding is `legacy-unbound`; bootstrap must not retrofit, infer, or reseal it.
...
5. Run `bash tools/tests/test-foundation.sh` and `bash tools/tests/test-app-bootstrap.sh all`. In the template, the bootstrap suite checks current source; a separate fixture regression checks compatibility and the source Identity file set, and a failure calls for `ruby tools/tests/lib/bootstrap-fixture.rb refresh .` from the template root. In a derived repository, including immediately after an uncommitted bootstrap, the same suite assembles a disposable repository from the fixture without changing app files, the index, or the Identity record. Confirm the generated repository retains the shape, harden, and release Issue contracts, validators, bounded verification tools, and repository tests. Exercise both a valid shape contract and a valid release contract in the generated sample. Run Xcode project listing, build, Unit Test, and UI Test through the repository's bounded wrappers with DerivedData and result bundles under `/tmp`, outside File Provider-managed repository paths. Verify that all targets and configurations retain the separately specified Deployment Target.
6. Resolve the latest installed iOS Runtime and invoke the canonical `ios-verify` route described in [`docs/verification.md`](../../../docs/verification.md). Run and visually evaluate the four fixed Simulator cases: latest iPhone Pro in English and Japanese, and latest iPad Air in English and Japanese. The inherited `tools/with-ios-simulator-lock.sh` session and `tools/lib/ios-simulator-resource.rb` own one device at a time while the Mac-wide cap remains four across repositories. For each condition, save Head-SHA-bound evidence outside the device, release and confirm exact device/data deletion, then allocate the next condition; do not create a four-device pool or delete an unmanaged device.
