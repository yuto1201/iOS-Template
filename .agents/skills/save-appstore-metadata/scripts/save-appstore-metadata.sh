...
if [[ ${IOS_TEMPLATE_TEST_MODE:-} == 1 ]]; then
  [[ -n ${IOS_TEMPLATE_TEST_ASC_RUNNER:-} ]] || exit 1
  exec /usr/bin/env -i PATH=/usr/bin:/bin LANG=en_US.UTF-8 HOME="$HOME" \
    ...
    --stage appstore-metadata-save --timeout-seconds 900 -- \
    /usr/bin/ruby --disable-gems -E UTF-8 "$tool_root/tools/lib/appstore-metadata-save.rb" "$@"
fi
[[ -z ${IOS_TEMPLATE_TEST_ASC_RUNNER:-} ]] || exit 1
exec /usr/bin/env -i PATH=/usr/bin:/bin LANG=en_US.UTF-8 HOME="$HOME" \
  /usr/bin/ruby --disable-gems "$tool_root/tools/lib/bounded-command.rb" \
  --stage appstore-metadata-save --timeout-seconds 900 -- \
  /usr/bin/ruby --disable-gems -E UTF-8 "$tool_root/tools/lib/appstore-metadata-save.rb" "$@"
