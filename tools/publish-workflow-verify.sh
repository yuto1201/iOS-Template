...
usage() {
  echo "usage: tools/publish-workflow-verify.sh --issue NUMBER --expected-base SHA --expected-head SHA --input PATH" >&2
  exit 64
}
...
exec /usr/bin/swift "$validator" --publish-workflow "$@"
