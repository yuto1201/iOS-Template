...
module IOSTemplate
  module IssueContract
    ...
    def parse_verification_scope(lines, headings, failures)
      ...
      if matches.length != 1
        failures << "duplicate optional heading: Verification scope"
        return nil
      end
    ...
    def validate_verification(value, acceptance_ids, failures, case_ids = VERIFICATION_CASE_IDS)
      ...
      if cases.is_a?(Array) && cases.length == case_ids.length
        ...
        failures << "Verification cases must contain the exact #{case_ids.length == 4 ? 'four' : 'one'} ordered case IDs"
      end
      ...
      mappings.each_with_index do |mapping, index|
        ...
        unless checks.is_a?(Array) && !checks.empty? && checks.uniq == checks &&
               checks.all? { |check| allowed_checks.include?(check) } &&
               checks == allowed_checks.select { |check| checks.include?(check) } &&
    ...
    def validate_snapshot!(value, issue:, repository:)
      ...
      scope_name = "full"
      begin
        scope_name = VerificationScope.validate_contract!(value)
      rescue ArgumentError => error
        failures << error.message
      end
