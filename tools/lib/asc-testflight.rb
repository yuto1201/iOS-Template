...
module IOSTemplate
  module AscTestFlight
    ...
    def validate_options!(options)
      ...
      refuse('what-to-test-unavailable-in-pinned-asc') if options[:notes_locale] || options[:notes_source]
    end
    ...
    def main(argv)
      ...
      parser = OptionParser.new do |cli|
        ...
        cli.on('--whats-new-locale LOCALE') { |value| options[:notes_locale]=value }
        cli.on('--whats-new-source PATH') { |value| options[:notes_source]=value }
      end
      parser.parse!(argv)
