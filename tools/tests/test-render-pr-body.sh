...
CONTRACT="$contract" VERIFY="$verify" STATE="$issue_dir/state.json" ruby -I "$primary/tools/lib" -rjson -rdigest -rissue-contract -e '
contract_path=ENV.fetch("CONTRACT"); contract=JSON.parse(File.binread(contract_path)); contract.delete("verification"); contract["deliveryProfile"]={"name"=>"fast","reason"=>"Non-UI low-risk renderer fixture."}; File.binwrite(contract_path,IOSTemplate::IssueContract.canonical_json(contract)); digest="sha256:#{Digest::SHA256.file(contract_path).hexdigest}"; verify_path=ENV.fetch("VERIFY"); verify=JSON.parse(File.binread(verify_path)); verify.fetch("issueContract")["digest"]=digest; File.binwrite(verify_path,JSON.pretty_generate(verify)+"\n"); state_path=ENV.fetch("STATE"); state=JSON.parse(File.binread(state_path)); state.fetch("issueContract")["digest"]=digest; File.binwrite(state_path,JSON.generate(state))'
