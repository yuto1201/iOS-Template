...
usage() {
  echo 'usage: tools/record-release-disposition.sh --issue NUMBER --base-sha SHA --head-sha SHA --input PATH' >&2
  exit 64
}
...
exec /usr/bin/ruby "$script_dir/lib/release-disposition-cli.rb" "$repo_root" "$issue" "$base_sha" "$head_sha" "$input"
