...
elif [[ -d "$repo_root/.artifacts" && ! -L "$repo_root/.artifacts" ]]; then
...
if [[ -d "$worktree" ]]; then
  "$worktree/tools/github-account-preflight.sh" --repo "$repo" --issue "$issue" --intended-operation github.delete_branch --expected-head "$head_sha" >/dev/null
fi
...
if [[ -e "$worktree" ]]; then git -C "$primary_root" worktree remove "$worktree" >/dev/null; fi
if git -C "$primary_root" show-ref --verify --quiet "refs/heads/$branch"; then git -C "$primary_root" branch -D -- "$branch" >/dev/null; fi
