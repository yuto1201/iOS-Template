...
workflow_require_sealed_issue_operation "$repo_root" "$repo" "$issue" github.read_issue 1 || blocked 'sealed Issue contract does not authorize Issue reads'
workflow_require_live_body_matches_sealed "$repo_root" "$repo" "$issue" "$issue_json" 1 || blocked 'live Issue body differs from the audited contract revision chain'
