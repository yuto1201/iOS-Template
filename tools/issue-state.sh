...
issue_json=$(gh issue view "$issue" --repo "$repo" --json labels,comments) || { echo 'Issue could not be read' >&2; exit 1; }
current=$(printf '%s' "$issue_json" | ruby "$json_tool" state-from-issue)
workflow_is_state "$current" || { echo 'Issue has an unknown current state label' >&2; exit 1; }
